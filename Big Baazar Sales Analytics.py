...
report['Day']=pd.Categorical(report['Day'],categories=week,ordered=True) #categorical function orders in the proper way for weekdays.
print(report)
report=report.sort_values('Day')
# weekdays=list(map(str, report['Day']))
weekdays=report.groupby(['Year','Day'])['sale_price_after_promo'].mean()
print(weekdays,'this is weekdays')

day_sale=report.groupby('Day')['sale_price_after_promo'].sum().sort_values(ascending=False)
print(day_sale)
day_sale.plot(kind='line')
